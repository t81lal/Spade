...
public class PipelineHeadStub<V> implements PipelineProducer<V, V>, PipelineComposable<V, V> {
    @Override
    public <T> PipelineComposed<V, V, T> then(PipelineStep<V, T> step) {
        return new ApplicativePipeline<>(this, this, step);
    }

    @Override
    public <T> PipelineComposed<V, V, T> then(Class<? extends PipelineStep<V, T>> stepClass) {
